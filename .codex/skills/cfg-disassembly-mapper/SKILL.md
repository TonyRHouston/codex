---
name: cfg-disassembly-mapper-7b3133
description: Use this skill when the user needs help with cfg disassembly mapper.
  It provides practical guidance, execution steps, and quality checks for cfg disassembly
  mapper tasks.
keywords:
- disassembly
- cfg
...
# cfg-disassembly-mapper

## When to use
- Use when the user request explicitly maps to **cfg disassembly mapper** outcomes.
...
When to use
- You need a fast function map and branch structure to guide deeper RE before decompilation.

Workflow
1. Radare2: `r2 -A target` then `afl` (list functions), `pdf @ main`, `s sym.func`, `pdf` for targeted disassembly.
2. Capstone scripting: recursively disassemble from an entry address, following jumps/calls to identify functions and back-edges (loops).
3. Mark interesting blocks (dispatchers, crypto loops) for decompiler focus.

Dependencies
- radare2; optional Python+Capstone for scripted traversal.

Output
- Function list and annotated control-flow hotspots to feed into decompilation.
